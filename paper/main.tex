...
point; 500 at $d{=}2$, $m{=}12$ and at $d{=}3$, $m{=}8$).}
\label{fig:scaling}
\end{figure*}
...
because the qubit decays roughly $4\times$ faster per precision bit:
slopes $-0.050\pm0.006$/bit ($d{=}2$, $R^2=0.97$, $n=4$ sizes) against
$-0.012\pm0.004$/bit ($d{=}3$, $R^2=0.72$, $n=5$) under the calibrated
...
($-0.012\pm0.004$/bit, $2.7\sigma$ from zero), while under
depolarizing the trend remains flat to the largest size
($+0.003\pm0.004$/bit, with signal $0.68\pm0.02$ at 12.7 bits). We
...
overtakes the ququint at 7.4
bits (calibrated ladder) and 10.5 bits (depolarizing), and from the
...
$-0.038\pm0.001$ vs $-0.040\pm0.004$). What does not replicate is the
qutrit's slope magnitude: $-0.025\pm0.004$/bit on $N=29$ against
$-0.012\pm0.004$ at $N=21$ (five sizes)---the shallowest slope in
every regime on both instances, but with instance-dependent
magnitude, so strict flatness survives only in the depolarizing
channel at $N=21$. What is instance-robust is the ordering, the
...
the qubit's fastest decay replicate across the instances; the qutrit's
slope magnitude does not ($-0.012$ vs $-0.025$/bit under the
calibrated ladder), so individual per-base slopes carry
instance-level variance of order $0.01$/bit. A fifth qutrit size
($m=8$, 12.7 bits) resolves the $N=21$ calibrated-ladder trend to
slow rather than flat; strict flatness persists only under
...
$d{=}2$, $m{=}12$, and at the fifth qutrit size $d{=}3$, $m{=}8$);
