...
\texorpdfstring{\\}{ }in uncorrected quantum algorithms under decoherence}
...
One condition organizes all three algorithms: \emph{qudits
...
($R^2=0.97$--$0.99$), as first-order composition of incoherent channels
requires; what the algorithm contributes is its decoder. For
continued-fraction order recovery we derive that contribution
exactly---the acceptance set of a decoder required to return $r$ itself
is a totient sum over the denominators it admits---prove it, and verify
it outcome-for-outcome on 27 instance/size combinations. Being
base-independent at matched control dimension, it places the entire
cross-base difference in the quantum state.
...
($R^2=0.97$--$0.99$, amplitude ${\approx}1$)---what first-order
...
tolerance obeys an exact law we derive and prove---the acceptance set
of a decoder required to return $r$ itself is a totient sum over the
denominators it admits, including the deeper convergents $kr\le N$ that
window bounds do not count---verified outcome-for-outcome against the
decoder on 27 instance/size combinations, with the underlying
acceptance lemma checked on 42
...
a determinate answer, and it is conditional. Width-and-depth
...
decomposition spends it back, and unmitigated structured dephasing
overwhelms it. What the algorithm itself adds is classical---a
decoder whose error tolerance obeys an exact totient law, identical
...
benchmark inherits these terms, and the cheapest decisive test,
eigenstate QPE at $d=5$ on a trapped-ion qudit processor, is within
reach of current hardware.
