...
\title{Native gates are necessary but not sufficient: the conditions for a%
\texorpdfstring{\\}{ }qudit advantage in uncorrected quantum algorithms under decoherence}
...
single arithmetic primitives under local noise, or error-correcting
codes, which have no problem instance to compress.
...
Hilbert-space dimension $5.3\times10^{5}$ (19.0 qubit-equivalents).
Two conditions organize all three algorithms: \emph{qudits
outperform qubits only with a native two-qudit entangling gate whose
cost grows no faster than linearly in $d$, and only while that gate's
own infidelity growth with $d$ stays below a critical inflation
factor $f^\ast$ we compute}. Gates compiled by two-level
decomposition forfeit the advantage in every case tested but one:
Shor at $d=3$ under per-particle noise, where width compression
alone survives the depth surcharge. The second condition is decided
by measured hardware: feeding the only published native two-qudit
entangling gate ($99.6/98.7/93.7\%$ at $d=2/3/5$) through our
threshold, the ququint advantage is lost in every channel/cost
pairing---even granting the gate a $1\sigma$-better fidelity---while
the qutrit retains a diminished advantage in the matched
trapped-ion pairing and straddles break-even elsewhere. Whether
linear cost suffices is further set by the level and structure of the
operating dephasing: free-evolution ladder dephasing splits the
linear-cost cells; steepening the per-level dephasing exponent to the
range the deepest published transmon ladder data require leaves only
$d=3$ in advantage, and only with a native gate; and unmitigated
Zeeman-structured dephasing reverses the verdict under a Markovian
generator but not under the quasi-static field noise laboratories
actually have, whose damage grows with the square of circuit depth and
so favors the shallower qudit schedule. Refocusing is where that
regime turns $d$-dependent instead: on the $^{40}$Ca$^{+}$ encoding a
single echo cancels the qubit's dephasing exactly but cannot cancel
any $d\ge3$, for which exact refocusing needs a $d$-interval pulse
sequence---and charging those pulses returns the advantage to $d=3$.
Along
...
predicts the winner in five of the six biased instance-channel cells
tested (three instances $\times$ two noise models), the sixth missing
by $1.4\sigma$. Accumulated channel damage fixes end-state
fidelity on a single exponential across algorithms and bases
($R^2=0.97$--$0.99$ in linear fidelity; $0.76$ rescored in log
fidelity), as first-order composition of incoherent channels
requires; what the algorithm contributes is its decoder. For
continued-fraction order recovery we give that contribution in closed
form: an exact finite-size mediant-interval formula, built on a proved
acceptance lemma and verified outcome-for-outcome on 27 instance/size
combinations, whose asymptotic density is a totient sum over the
denominators the decoder admits. Being
base-independent at matched control dimension, it places the entire
cross-base difference in the quantum state. A corollary---noiseless
aligned success is exactly $\varphi(r)/r$---explains which instances
can be scored at all.
The qubit branch of our predictions is checked on hardware: the
shallow compiled circuit lands inside its predicted success band on a
commercial trapped-ion processor ($0.617\pm0.007$ vs $0.60$--$0.70$),
consistent with the vendor's measured per-gate infidelity, while two
deeper circuits on the same device fail coherently, bounding the
convention's regime of validity at shallow depth.
\end{abstract}
...
hardware currently decides the second condition against the ququint:
the one published native two-qudit gate exceeds $f^\ast$ at $d=5$ in
every pairing, while the qutrit sits at the boundary. Free-evolution
ladder dephasing further splits the linear-cost cells by algorithm and
dimension, and unmitigated Zeeman-structured dephasing---higher levels
dephasing tens of times faster---reverses the verdict at every cost
model (Secs.~\ref{sec:cost} and~\ref{sec:robustness}).}
\end{quote}
...
infidelity condition is what that same gate currently fails: its
measured error grows $16\times$ from $d=2$ to $d=5$
($0.004\to0.063$), and Sec.~\ref{sec:robustness} shows this exceeds
the critical inflation factor at $d=5$ under every accounting.
...
algorithm-level outcomes in seven of nine cost-model/dimension cases
(both misses in the conservative direction), with break-even
...
retains it ($16p^2n$ for the in-place constant multiplier) under a
1D-local-neighbor architecture with the same two-level decomposition
assumption, supporting our treatment of \texttt{pavlidis} as a
uniform layer multiplier). The three
...
$d=7$ demo grid, $0.385$ vs $0.338$). The exception isolates the width component of the
...
criterion is consistent with our algorithm-level winner in seven of nine
...
extrapolation beyond any measurement. The decomposition direction is unchanged: \texttt{pavlidis}
...
\emph{The threshold at measured fidelities.} The one published native
two-qudit entangling gate---the single-application light-shift gate
of Hrmo \emph{et al.}~\cite{hrmo2023}, fidelity $99.6(1)/98.7(2)/
93.7(3)\%$ at $d=2/3/5$---decides the second condition. Converting
each measured infidelity $\epsilon$ to the implied per-carrier-layer
strength $s=\epsilon/[2L(1-1/d^2)]$ and forming $f=s_d/s_2$: under
\texttt{uniform} accounting (the gate charged one layer, as its
single-application construction warrants) $f=2.74\pm0.80$ at $d=3$
and $12.30\pm3.13$ at $d=5$; under the \texttt{ion} accounting (the
$(d{-}1)$ layer charge absorbing part of the growth)
$f=1.37\pm0.40$ and $3.08\pm0.78$ ($1\sigma$ propagated from the
quoted gate errors). Rerunning the demo grid by exact density
matrices at these measured factors (Table~\ref{tab:hrmo}): \emph{the
ququint advantage is lost in every channel/cost pairing, even
granting the gate a $1\sigma$-better fidelity}---at $d=5$ the
measured $f$ exceeds $f^\ast$ by $2.7\times$ (uniform, depolarizing)
to $5\times$ (ladder)---\emph{while the qutrit survives in exactly
one pairing}, per-particle depolarizing under \texttt{ion} cost
($f=1.37<f^\ast=1.56$; signal $0.38$ vs the qubit's $0.33$), and
loses centrally but within $1\sigma$ in the other three. Native
gates are therefore necessary but not, at measured fidelities,
sufficient: the condition the title states has a second clause, and
current hardware fails it at $d=5$ and straddles it at $d=3$.
...
${\approx}f^\ast\cdot 8\,(0.96/0.75)\,\epsilon_2\approx0.033$ at
...
density matrices; \texttt{results/hrmo\_reanalysis.json}).
``$f$ measured'' is the inflation factor implied by the gate
fidelities under each accounting ($\pm1\sigma$); signals are
...
\hline
ladder & \texttt{uniform} & 3 & $2.7\pm0.8$ & 2.05 & 0.39 (0.51) \\
       & \texttt{uniform} & 5 & $12.3\pm3.1$ & 2.46 & 0.01 (0.51) \\
       & \texttt{ion}     & 3 & $1.4\pm0.4$ & 1.21 & 0.45 (0.51) \\
       & \texttt{ion}     & 5 & $3.1\pm0.8$ & --- & 0.08 (0.51) \\
depol. & \texttt{uniform} & 3 & $2.7\pm0.8$ & 2.65 & 0.32 (0.33) \\
       & \texttt{uniform} & 5 & $12.3\pm3.1$ & 4.50 & 0.05 (0.33) \\
...
falls from $0.690$ to $0.073$ under \texttt{uniform} cost across the
range, and is below the qubit from exponent $2.0$ on. The dimension
...
$26.3\%/23.1\%/20.0\%$ at $d=2/3/5$ under \texttt{uniform} cost and
$26.3\%/15.9\%/7.1\%$ under \texttt{ion}, confirming the predicted
direction: the qubit gains most, and under linear cost the ququint
gains almost nothing. The effect on verdicts is nonetheless
nil. Re-running the six cells of Table~\ref{tab:cost} under the
parallel schedule leaves \emph{all six} qudit-vs-qubit verdicts
unchanged, while every margin erodes: $+0.452\to+0.385$ and
$+0.172\to+0.113$ (depolarizing, \texttt{uniform}/\texttt{ion}),
$+0.349\to+0.268$ and $+0.092\to+0.023$ (ladder), and the
\texttt{pavlidis} qubit win deepens from $-0.027$ to $-0.055$. Two
readings follow. The serial convention inflates the qudit margin by up
to a factor of four but does not create it; and the ladder/\texttt{ion}
...
threshold rather than a table row. Three of the six cells survive the
steeper calibration: depolarizing under \texttt{uniform}
($\alpha^\ast$ beyond the swept range) and \texttt{ion}
($\alpha^\ast=3.55$), and the ladder under \texttt{uniform}
($\alpha^\ast=3.84$). Two do not, and both matter. The
ladder/\texttt{ion} cell has $\alpha^\ast=2.14$---it fails at
\emph{exactly} the measured pulse-count charge, margin
$+0.092\to-0.001$. And the depolarizing/\texttt{pavlidis} exception,
the one cell where a qudit beats the qubit under the harshest
entangling cost, has $\alpha^\ast=1.16$ and does not survive any
measured charge ($+0.063\to-0.069$ at $\alpha=2.15$). Across the whole
...
brackets the vendor's measured $0.7\%$ two-qubit infidelity. We read
this as a single-point consistency check of the noise convention at
shallow depth---one scalar cannot validate the channel's structure,
and SPAM, single-qubit error, and idle decoherence are all absorbed
into the one fitted parameter---while the two deeper circuits below
...
\emph{qutrit}: the one pairing that survives at measured fidelities
(\texttt{ion} cost, per-particle noise, $f=1.37<f^\ast=1.56$) is
exactly this platform, the margins are small enough that either
...
schedule no cell of Table~\ref{tab:cost} changes verdict, though
margins shrink by up to $4\times$. The scaling trends are measured
...
decomposition spends it back, the native gate's own infidelity growth
with $d$ can spend it back just as surely---at measured fidelities it
does, for the ququint, in every pairing---and unmitigated structured
dephasing overwhelms it. What the algorithm itself adds is classical---a
...
processor---the one pairing measured fidelities leave alive---while
...
points characterized above. Two claims did move, and both are stated
in their corrected form in the text rather than here: one of the six
grid-alignment predictions now misses by $1.4\sigma$
(Sec.~\ref{sec:alignment}), and the qutrit's depolarizing slope no longer
replicates across instances (Sec.~\ref{sec:scaling}). No cost-table
or robustness verdict changed---those are exact density matrices and
carry no seed. Every paired shift is recorded in
\texttt{results/prehash\_comparison.json}, and the size-scaling fits
are regenerated by \texttt{scaling\_claims.py}. Fractional gate costs are exact: both channel families form
