...
Zeeman-structured dephasing---a regime measured devices shield and
refocus away---reverses the verdict outright. Along
...
from 6.3 to 9.5 bits; the fit's $\chi^2/\mathrm{dof}=0.01$ indicates
the trajectory error bars are conservative for these points, so we
quote the raw spread rather than a significance) and the whole
decline is carried by the last
three, the 14.3-bit point sitting $4.1\sigma$ below the 9.5-bit one
on the same conservative bars.
...
\begin{table}[t]
\caption{Zeeman-structured dephasing on the demo instance:
floor-corrected signal at $s=0.003$, \texttt{uniform} cost. Both
variants reverse the qudit ordering of Table~\ref{tab:cost}; the
reversal persists under \texttt{ion} cost.}
\label{tab:zeeman}
\begin{ruledtabular}
\begin{tabular}{lccc}
...
include. Two caveats bound what this worst case establishes. First,
the Markovian generator is a stand-in: laboratory field noise is
quasi-static, decays as a Gaussian rather than an exponential at
short times, and is removed to first order by a single spin
echo---properties the Lindblad substitution erases, so the unmitigated
row overstates the persistence of an error that refocusing is
routinely applied against. Second, mitigation with overhead
\emph{constant in $d$} has been proposed: passive dynamical
decoupling by rotation of the quantization axis decouples every
magnetically sensitive sublevel simultaneously~\cite{sutherland2023}.
The operational content of this subsection is therefore not the
unmitigated reversal but the suppression bar computed next---how
much mitigation the qudit ordering requires.
